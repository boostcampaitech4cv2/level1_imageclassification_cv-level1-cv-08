...
def main(config, filename):
    ...
    for images in tqdm(loader):
        with torch.no_grad():
            ...
            # elif pred_age < 60: pred = encoder(mask, gender, 1)
            # else:               pred = encoder(mask, gender, 2)
            ...
            #for age correction
            # mask, gender, age = decoder(preds)
            # # print(mask, gender, age)
            # outputs_age = model_age(images)
            # _, preds_age = torch.max(outputs_age, 1)
            # preds = encoder(mask, gender, preds_age)
...
if __name__ == '__main__':
    ...
    args.add_argument('-c', '--config', default='cfg/efficient.json', type=str,
                      help='config file path (default: None)')
    args.add_argument('-r', '--resume', default='saved/models/efficent/1027_011536/model_best.pth', type=str,
                      help='path to latest checkpoint (default: None)')
    args.add_argument('-d', '--device', default=None, type=str,
                      help='indices of GPUs to enable (default: all)')
    args.add_argument('-n', '--name', default='eff_22_fin_agecorr', type=str)
...
#     val_top_k_acc  : 1.0
# Validation performance didn't improve for 10 epochs. Training stops.    
#python submit_age.py --c cfg/efficient.json --r saved/models/efficent/1027_011536/model_best.pth --n eff_22_fin_agecorr_100
#python train.py --c cfg/efficient_age_3_ce_b7.json